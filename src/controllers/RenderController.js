...
async function renderVideo(expressApp, data) {
  ...
  const timeout = data.timeline.fitDuration * 2 * 1000;
...
function mergeFinalVideoFromChunks(randomId, chunksPaths, fitDuration) {
  return new Promise(async (resolve, reject) => {
    ...
    const audioChunk = chunksPaths.pop();
    ...
    const ffmpegCmd = `ffmpeg -f concat -safe 0 -i ${tempChunksInputFile} -ss 00:00:00 -t ${fitDuration} -i ${audioChunk}  -c:v copy -c:a aac -map 0:v:0 -map 1:a:0 ${outputFilePath}`;
